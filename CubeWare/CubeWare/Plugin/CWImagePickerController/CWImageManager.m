...
@interface CWImageManager ()
@property (nonatomic, strong) ALAssetsLibrary *assetLibrary;
@end

@implementation CWImageManager
static CGSize AssetGridThumbnailSize;
static CGFloat CWScreenWidth;
static CGFloat CWScreenScale;

+ (instancetype)manager {
    static CWImageManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[self alloc] init];
        manager.cachingImageManager = [[PHCachingImageManager alloc] init];
        manager.cachingImageManager.allowsCachingHighQualityImages = NO;
        CWScreenWidth = [UIScreen mainScreen].bounds.size.width;
        // 测试发现，如果scale在plus真机上取到3.0，内存会增大特别多。故这里写死成2.0
        CWScreenScale = 2.0;
        if (CWScreenWidth > 700) {
            CWScreenScale = 1.5;
        }
        CGFloat margin = 4;
        CGFloat itemWH = (CWScreenWidth - 2 * margin - 4) / 4 - margin;
        AssetGridThumbnailSize = CGSizeMake(itemWH * CWScreenScale, itemWH * CWScreenScale);
    });
    return manager;
}
...
/// Get Album 获得相册/相册数组
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(CWAlbumModel *))completion{
    __block CWAlbumModel *model;
    if (IOS8) {
...
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<CWAlbumModel *> *))completion{
...
/// Get Assets 获得照片数组
- (void)getAssetsFromFetchResult:(id)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<CWAssetModel *> *))completion {
    NSMutableArray *photoArr = [NSMutableArray array];
    if ([result isKindOfClass:[PHFetchResult class]]) {
        PHFetchResult *fetchResult = (PHFetchResult *)result;
        [fetchResult enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            PHAsset *asset = (PHAsset *)obj;
            CWAssetModelMediaType type = CWAssetModelMediaTypePhoto;
            if (asset.mediaType == PHAssetMediaTypeVideo)      type = CWAssetModelMediaTypeVideo;
            else if (asset.mediaType == PHAssetMediaTypeAudio) type = CWAssetModelMediaTypeAudio;
            else if (asset.mediaType == PHAssetMediaTypeImage) {
                if (iOS9_1) {
                    // if (asset.mediaSubtypes == PHAssetMediaSubtypePhotoLive) type = CWAssetModelMediaTypeLivePhoto;
                }
            }
            if (!allowPickingVideo && type == CWAssetModelMediaTypeVideo) return;
            if (!allowPickingImage && type == CWAssetModelMediaTypePhoto) return;
            
            NSString *timeLength = type == CWAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
            timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
            [photoArr addObject:[CWAssetModel modelWithAsset:asset type:type timeLength:timeLength]];
        }];
        ...
        [group enumerateAssetsUsingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
            ...
            if (!allowPickingVideo){
                [photoArr addObject:[CWAssetModel modelWithAsset:result type:type]];
                return;
            }
            /// Allow picking video
            if ([[result valueForProperty:ALAssetPropertyType] isEqualToString:ALAssetTypeVideo]) {
                type = CWAssetModelMediaTypeVideo;
                NSTimeInterval duration = [[result valueForProperty:ALAssetPropertyDuration] integerValue];
                NSString *timeLength = [NSString stringWithFormat:@"%0.0f",duration];
                timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
                [photoArr addObject:[CWAssetModel modelWithAsset:result type:type timeLength:timeLength]];
            } else {
                [photoArr addObject:[CWAssetModel modelWithAsset:result type:type]];
            }
        }];
...
- (void)getAssetFromFetchResult:(id)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(CWAssetModel *))completion {
    if ([result isKindOfClass:[PHFetchResult class]]) {
        PHFetchResult *fetchResult = (PHFetchResult *)result;
        ...
        NSString *timeLength = type == CWAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
        timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
        CWAssetModel *model = [CWAssetModel modelWithAsset:asset type:type timeLength:timeLength];
        if (completion) completion(model);
    } else if ([result isKindOfClass:[ALAssetsGroup class]]) {
        ALAssetsGroup *group = (ALAssetsGroup *)result;
        if (allowPickingImage && allowPickingVideo) {
        ...
        [group enumerateAssetsAtIndexes:indexSet options:NSEnumerationConcurrent usingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
            ...
            CWAssetModelMediaType type = CWAssetModelMediaTypePhoto;
            if (!allowPickingVideo){
                model = [CWAssetModel modelWithAsset:result type:type];
                if (completion) completion(model);
                return;
            }
            /// Allow picking video
            if ([[result valueForProperty:ALAssetPropertyType] isEqualToString:ALAssetTypeVideo]) {
                ...
                NSString *timeLength = [NSString stringWithFormat:@"%0.0f",duration];
                timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
                model = [CWAssetModel modelWithAsset:result type:type timeLength:timeLength];
            } else {
                model = [CWAssetModel modelWithAsset:result type:type];
            }
            if (completion) completion(model);
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *totalBytes, NSInteger totalLength))completion {
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        CWAssetModel *model = photos[i];
        if ([model.asset isKindOfClass:[PHAsset class]]) {
            [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
                if (model.type != CWAssetModelMediaTypeVideo) dataLength += imageData.length;
                assetCount ++;
            ...
            if (model.type != CWAssetModelMediaTypeVideo) dataLength += (NSInteger)representation.size;
            if (i >= photos.count - 1) {
                NSString *bytes = [self getBytesFromDataLength:dataLength];
                if (completion) completion(bytes, dataLength);
            }
        ...
        if ([model.asset isKindOfClass:[PHAsset class]]) {
            if (model.type != CWAssetModelMediaTypeVideo) {
                [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
                    ...
                    if (dataLength > imageLength) {
                        completion([NSString stringWithFormat:@"发送单个图片大小不能超出%@,请重新选择", [self getIntBytesFromDataLength:imageLength]]);
                    } else if (image.size.width > imageSize.width || image.size.width > imageSize.height) {
                        completion([NSString stringWithFormat:@"发送单个图片分辨率不能超出%d*%d,请重新选择", (int)imageSize.width, (int)imageSize.height]);
                    } else{
                        completion(nil);
                    }
                }];
                ...
                [[PHImageManager defaultManager] requestAVAssetForVideo:model.asset options:options resultHandler:^(AVAsset *asset, AVAudioMix *audioMix, NSDictionary *info) {
                    if ([asset isKindOfClass:[AVURLAsset class]]) {
                        ...
                        if ([size integerValue] > imageLength) {
                            completion([NSString stringWithFormat:@"发送单个图片大小不能超出%@,请重新选择", [self getIntBytesFromDataLength:imageLength]]);
                        }
                    }
                }];
            ...
            if (model.type != CWAssetModelMediaTypeVideo){
                ...
                if (dataLength > 10 * (1024 * 1024)) {
                    completion(@"发送单个图片大小不能超出10M,请重新选择");
                } else if (dimension.width > 4096 || dimension.width > 4096) {
                    completion(@"发送单个图片分辨率不能超出4096*4096,请重新选择");
                } else{
                    completion(nil);
                }
            }
...
- (NSString *)getIntBytesFromDataLength:(NSInteger)dataLength {
    ...
    if (dataLength >= 1 * 1024 * 1024) {
        bytes = [NSString stringWithFormat:@"%dM",(int)(dataLength/1024/1024)];
    } else if (dataLength >= 1 * 1024) {
        bytes = [NSString stringWithFormat:@"%dK",(int)dataLength/1024];
    } else {
        bytes = [NSString stringWithFormat:@"%dB",(int)dataLength];
    }
    return bytes;
}
...
- (PHImageRequestID)getPhotoWithAsset:(id)asset completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    CGFloat fullScreenWidth = CWScreenWidth;
...
- (PHImageRequestID)getPhotoWithAsset:(id)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    if ([asset isKindOfClass:[PHAsset class]]) {
        CGSize imageSize;
        if (photoWidth < CWScreenWidth && photoWidth < _photoPreviewMaxWidth) {
            imageSize = AssetGridThumbnailSize;
        } else {
            PHAsset *phAsset = (PHAsset *)asset;
            CGFloat aspectRatio = phAsset.pixelWidth / (CGFloat)phAsset.pixelHeight;
            CGFloat pixelWidth = photoWidth * CWScreenScale;
...
- (void)getPostImageWithAlbumModel:(CWAlbumModel *)model completion:(void (^)(UIImage *))completion {
    if (IOS8) {
        [[CWImageManager manager] getPhotoWithAsset:[model.result lastObject] photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (void)savePhotoWithImage:(UIImage *)image completion:(void (^)())completion {
    ...
    if (iOS9) { // 这里有坑... iOS8系统下这个方法保存图片会失败
...
- (CWAlbumModel *)modelWithResult:(id)result name:(NSString *)name{
    CWAlbumModel *model = [[CWAlbumModel alloc] init];
    model.result = result;
