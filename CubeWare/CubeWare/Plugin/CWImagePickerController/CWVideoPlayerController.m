...
@interface CWVideoPlayerController (){
    AVPlayer *_player;
...
@implementation CWVideoPlayerController
...
- (void)configMoviePlayer {
    [[CWImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        _cover = photo;
    }];
    [[CWImageManager manager] getVideoWithAsset:_model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
...
- (void)configPlayButton {
    ...
    _playButton.frame = CGRectMake(0, 64, self.view.cw_width, self.view.cw_height - 64 - 44);
#warning 图片资源找不到
    [_playButton setImage:[CWResourceUtil imageNamed:@"MMVideoPreviewPlay.png"] forState:UIControlStateNormal];
#warning 图片资源找不到
    [_playButton setImage:[CWResourceUtil imageNamed:@"MMVideoPreviewPlayHL.png"] forState:UIControlStateHighlighted];
    [_playButton addTarget:self action:@selector(playButtonClick) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:_playButton];
}

- (void)configBottomToolBar {
    _toolBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.cw_height - 44, self.view.cw_width, 44)];
